  ...
  protected formatPopover(
    ...
    switch (t[0]) {
      ...
      case 'rset':
        poData.id = id;
        poData.type = t[0];
        // Récupérer le ResourceSet complet (pas seulement la feature)
        const rsetItem = this.skresOther.fromCache(id, false);
        poData.resource = rsetItem;
        poData.title = rsetItem?.name ?? 'Resource Set';
        poData.show = rsetItem ? true : false;
        poData.readOnly = false; // Permettre la suppression des resource sets
        break;
      default:
        return;
    }
  ...
  protected deleteFeature(id: string, type: string) {
    switch (type) {
      ...
      case 'region':
        ...
        break;
      case 'rset':
        // Cette méthode doit supprimer le fichier du serveur
        this.skresOther.deleteResourceSet(id);
        break;
    }
  }
