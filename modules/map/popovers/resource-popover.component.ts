  ...
  template: `
    ...
    >
      ...
      <div style="display:flex;flex-wrap: wrap;">
        ...
        } @if(ctrl.showAlertButton) {
        <div class="popover-action-button">
          <button
            mat-button
            (click)="emitToggleAlert()"
            [matTooltip]="ctrl.alertEnabled ? 'Disable Alert' : 'Enable Alert'"
            matTooltipPosition="after"
            [disabled]="ctrl.isReadOnly"
          >
            <mat-icon>{{ ctrl.alertEnabled ? 'notifications_active' : 'notifications_off' }}</mat-icon>
            {{ ctrl.alertEnabled ? 'ALERT ON' : 'ALERT OFF' }}
          </button>
        </div>
...
export class ResourcePopoverComponent {
  ...
  @Output() toggleAlert: EventEmitter<boolean> = new EventEmitter();
  ...
  parseRegion() {
    ...
    this.ctrl.showPointsButton = false;
    this.ctrl.showRelatedButton = false;
    this.ctrl.showAlertButton = true;
    this.ctrl.isReadOnly = this.resource[1].feature.properties?.readOnly;
    this.properties = [];
    this.icon = getResourceIcon('regions', this.resource[1]);
    this.title = this.resource[1].name ?? '';
    
    this.ctrl.alertEnabled = this.resource[1].feature.properties?.alertEnabled ?? false;
    this.properties.push(['Alert', this.ctrl.alertEnabled ? 'Enabled' : 'Disabled']);
  }
  ...
  emitToggleAlert() {
    const newState = !this.ctrl.alertEnabled;
    this.toggleAlert.emit(newState);
  }
...
@Component({
  ...
  template: `
    <ap-popover [title]="title" [canClose]="canClose" (closed)="handleClose()">
      ...
      <div style="display:flex;">
        <div style="font-weight:bold;">{{ p[0] }}:</div>
        <div style="flex: 1 1 auto;text-align:right;white-space:nowrap;overflow-x:auto;text-overflow:ellipsis;">
          {{ p[1] }}
        </div>
      </div>
      } }
...
export class ResourceSetPopoverComponent {
  ...
  parse() {
    ...
    this.properties.push(['Name', name]);
    this.properties.push(['Description', description]);
  }
