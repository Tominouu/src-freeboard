...
export class SKOtherResources {
  ...
  /**
   * Delete a ResourceSet from the server
   * @param id Full resource set id (e.g., "rset.collection.resourceId")
   * @returns Promise that resolves when deletion is complete
   */
  public deleteResourceSet(id: string): Promise<void> {
    return new Promise((resolve, reject) => {
      const t = id.split('.');
      if (t[0] !== 'rset' || t.length < 3) {
        reject(new Error('Invalid ResourceSet ID format'));
        return;
      }
      ...
      const apiUrl = `/resources/${collection}/${resourceId}`;

      this.signalk.api.delete(this.app.skApiVersion, apiUrl).subscribe(
        () => {
          // Remove from cache
          this.resSetCacheSignal.update((current: FBResourceSets) => {
            const items = current.get(collection) || [];
            const filtered = items.filter((item) => item.id !== resourceId);
            current.set(collection, filtered);
            return current;
          });

          // Remove from selections
          if (Array.isArray(this.app.config.selections.resourceSets[collection])) {
            const index = this.app.config.selections.resourceSets[collection].indexOf(resourceId);
            if (index !== -1) {
              this.app.config.selections.resourceSets[collection].splice(index, 1);
              this.app.saveConfig();
            }
          }
